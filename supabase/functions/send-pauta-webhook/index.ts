...
serve(async (req) => {
  try {
    ...
    const { data: pauta, error: pautaError } = await supabase
      .from('pautas_events')
      .select('id, titulo, descricao, data_inicio, filmmaker_id, fotografo_id, jornalista_id, rede_id, editor_id, direcao_id, producao_id')
      .eq('id', pauta_id)
      .single()
    ...
    if (!pauta.filmmaker_id && !pauta.fotografo_id && !pauta.jornalista_id && !pauta.rede_id && !pauta.editor_id && !pauta.direcao_id && !pauta.producao_id) {
    ...
    const teamMemberIds = [
      ...
      { id: pauta.producao_id, funcao: 'Produção' }
