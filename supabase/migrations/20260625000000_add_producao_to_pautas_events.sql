-- ============================================================================
-- MIGRAÇÃO: Adicionar campo producao_id à tabela pautas_events
-- producao_id: UUID[] para múltiplos produtores (sem filtro de cargo)
-- ============================================================================

-- 1. Adicionar coluna como UUID[] (array) com default vazio
ALTER TABLE public.pautas_events
  ADD COLUMN IF NOT EXISTS producao_id UUID[] DEFAULT ARRAY[]::UUID[];

-- 2. Criar índice para melhor performance nas consultas
CREATE INDEX IF NOT EXISTS idx_pautas_events_producao ON public.pautas_events USING GIN (producao_id);

-- 3. Atualizar webhook function diário para incluir produção
...
BEGIN
  ...
  -- Buscar todas as pautas do dia seguinte (INCLUINDO DESCRIÇÃO E PRODUÇÃO)
  FOR pauta_record IN
    SELECT 
      id,
      titulo,
  ...
  LOOP
    ...
    IF pauta_record.producao_id IS NOT NULL AND array_length(pauta_record.producao_id, 1) > 0 THEN
      FOREACH member_id IN ARRAY pauta_record.producao_id
      LOOP
        SELECT full_name, phone INTO profile_record
        FROM public.profiles WHERE id = member_id;
        IF FOUND THEN
...
-- 4. Atualizar função de webhook direto para incluir produção
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    team_changed := (
      ...
      (NEW.direcao_id IS NOT NULL AND array_length(NEW.direcao_id, 1) > 0) OR
      (NEW.producao_id IS NOT NULL AND array_length(NEW.producao_id, 1) > 0)
    );
  ELSIF TG_OP = 'UPDATE' THEN
    team_changed := (
      ...
      (OLD.direcao_id IS DISTINCT FROM NEW.direcao_id) OR
      (OLD.producao_id IS DISTINCT FROM NEW.producao_id)
    );
  END IF;
  ...
  IF team_changed THEN
    ...
    -- Buscar produção
    IF NEW.producao_id IS NOT NULL AND array_length(NEW.producao_id, 1) > 0 THEN
      FOREACH member_id IN ARRAY NEW.producao_id
      LOOP
        SELECT full_name, phone INTO profile_record
        FROM public.profiles WHERE id = member_id;
        IF FOUND THEN
