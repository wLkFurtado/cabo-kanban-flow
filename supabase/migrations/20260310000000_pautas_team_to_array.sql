-- ============================================================================
-- MIGRAÇÃO: Converter campos de equipe da pauta de UUID (single) para UUID[] (array)
-- Permite múltiplas pessoas nas funções de filmmaker, fotografo, jornalista e rede
-- ============================================================================

-- 1. Remover constraint de FK (UUID referência única não se aplica a array)
ALTER TABLE pautas_events DROP CONSTRAINT IF EXISTS pautas_events_filmmaker_id_fkey;
ALTER TABLE pautas_events DROP CONSTRAINT IF EXISTS pautas_events_fotografo_id_fkey;
ALTER TABLE pautas_events DROP CONSTRAINT IF EXISTS pautas_events_jornalista_id_fkey;
ALTER TABLE pautas_events DROP CONSTRAINT IF EXISTS pautas_events_rede_id_fkey;

-- 2. Converter coluna: UUID → UUID[] preservando dados existentes
ALTER TABLE pautas_events
  ALTER COLUMN filmmaker_id TYPE UUID[]
  USING CASE WHEN filmmaker_id IS NOT NULL THEN ARRAY[filmmaker_id] ELSE ARRAY[]::UUID[] END;

ALTER TABLE pautas_events
  ALTER COLUMN fotografo_id TYPE UUID[]
  USING CASE WHEN fotografo_id IS NOT NULL THEN ARRAY[fotografo_id] ELSE ARRAY[]::UUID[] END;

ALTER TABLE pautas_events
  ALTER COLUMN jornalista_id TYPE UUID[]
  USING CASE WHEN jornalista_id IS NOT NULL THEN ARRAY[jornalista_id] ELSE ARRAY[]::UUID[] END;

ALTER TABLE pautas_events
  ALTER COLUMN rede_id TYPE UUID[]
  USING CASE WHEN rede_id IS NOT NULL THEN ARRAY[rede_id] ELSE ARRAY[]::UUID[] END;

-- 3. Definir default como array vazio
ALTER TABLE pautas_events ALTER COLUMN filmmaker_id SET DEFAULT ARRAY[]::UUID[];
ALTER TABLE pautas_events ALTER COLUMN fotografo_id SET DEFAULT ARRAY[]::UUID[];
ALTER TABLE pautas_events ALTER COLUMN jornalista_id SET DEFAULT ARRAY[]::UUID[];
ALTER TABLE pautas_events ALTER COLUMN rede_id SET DEFAULT ARRAY[]::UUID[];



-- 4. Atualizar webhook function diário para iterar os arrays da equipe
...
-- 5. Atualizar função de webhook direto para iterar arrays
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    team_changed := (
      ...
      (NEW.rede_id IS NOT NULL AND array_length(NEW.rede_id, 1) > 0)
    );
