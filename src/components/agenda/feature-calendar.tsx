...
const monthsForLocale = (
  ...
  const format = new Intl.DateTimeFormat(
    localeName as string | string[] | undefined,
    { month: monthFormat }
  ).format;
...
const daysForLocale = (
  ...
  for (let i = 0; i < 7; i++) {
    weekdays.push(
      new Intl.DateTimeFormat(locale as string | string[] | undefined, {
        weekday: "short",
      }).format(baseDate)
    );
...
export const CalendarMonthPicker = ({ className }: CalendarMonthPickerProps) => {
  ...
  return (
    <Combobox
      ...
      labels={{
        button: "Selecionar mês",
        empty: "Nenhum mês encontrado",
        search: "Buscar mês",
      }}
    />
  );
...
export const CalendarYearPicker = ({ className, start, end }: CalendarYearPickerProps) => {
  ...
  return (
    <Combobox
      ...
      labels={{
        button: "Selecionar ano",
        empty: "Nenhum ano encontrado",
        search: "Buscar ano",
      }}
    />
  );
...
// Export only components to satisfy react-refresh fast-refresh rules
// Consumers should import components individually