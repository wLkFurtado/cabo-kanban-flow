  ...
  const duplicateEventMutation = useMutation({
    mutationFn: async (eventId: string) => {
      ...
      const { data: originalEvent, error: fetchError } = await supabase
        .from('pautas_events')
        .select('id, titulo, descricao, data_inicio, data_fim, tipo, prioridade, status, recorrencia, cor, local, responsavel_id, filmmaker_id, fotografo_id, jornalista_id, rede_id, editor_id, direcao_id, producao_id, criado_por, created_at, updated_at')
        .eq('id', eventId)
        .single();
